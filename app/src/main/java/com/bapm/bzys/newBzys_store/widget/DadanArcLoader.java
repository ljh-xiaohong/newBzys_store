...
public class DadanArcLoader extends View implements Animatable {
	...
	public DadanArcLoader(Context context) {
		super(context);
	}

	public DadanArcLoader(Context context, AttributeSet attrs) {
		super(context, attrs);

		init(attrs);
	}

	public DadanArcLoader(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private ArcConfiguration readFromAttributes(AttributeSet attributeSet) {
		...
		TypedArray array = getContext().obtainStyledAttributes(attributeSet, R.styleable.DadanArcLoader);

		for (int i = 0; i < array.length(); i++) {
			int type = array.getIndex(i);

			if (type == R.styleable.DadanArcLoader_arc_style) {
				String value = array.getString(R.styleable.DadanArcLoader_arc_style);

				configuration.setLoaderStyle(STYLE.values()[Integer.parseInt(value)]);
			}

			if (type == R.styleable.DadanArcLoader_arc_colors) {
				int colors_resourse_id = array.getResourceId(R.styleable.DadanArcLoader_arc_colors, 0);

				if (colors_resourse_id != 0)
					configuration.setColors(getContext().getResources().getIntArray(colors_resourse_id));
			}

			if (type == R.styleable.DadanArcLoader_arc_speed) {
				String value = array.getString(R.styleable.DadanArcLoader_arc_speed);

				if (value != null)
					configuration.setAnimationSpeedWithIndex(Integer.parseInt(value));
			}

			if (type == R.styleable.DadanArcLoader_arc_margin) {
				Float value = array.getDimension(R.styleable.DadanArcLoader_arc_margin, MARGIN_MEDIUM);

				configuration.setArcMargin(value.intValue());
			}

			if (type == R.styleable.DadanArcLoader_arc_thickness) {
				Float value = array.getDimension(R.styleable.DadanArcLoader_arc_thickness,
						getContext().getResources().getDimension(R.dimen.dialog_stroke_width));
