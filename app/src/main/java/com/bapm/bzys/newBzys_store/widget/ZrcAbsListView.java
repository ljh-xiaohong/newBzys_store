...
import  com.bapm.bzys.newBzys_store.widget.ZrcListView.OnScrollListener;
import  com.bapm.bzys.newBzys_store.widget.ZrcListView.OnScrollStateListener;
import  com.bapm.bzys.newBzys_store.widget.ZrcListView.OnStartListener;
...
//				if (inList && !child.hasFocusable()) {
		...
		boolean scrollToAdjustViewsUpOrDown() {
			...
			if (cannotScrollDown) {
				...
				if ((zrcHeader != null && onRefreshStart != null && zrcHeader
						.getState() == Headable.STATE_RELEASE) || showHeader) {
					if (!isOnLoading) {
						isRefreshing = true;
						showHeader = true;
	...
	boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
		...
		if(childCount == 0){
			mFirstTop=mFirstTop + incrementalDeltaY;
...
//		mFirstTop = (childCount == 0 ? mFirstTop + incrementalDeltaY: getChildAt(0).getTop());
	...
	class RecycleBin {
		...
		void addScrapView(View scrap, int position) {
			final LayoutParams lp = (LayoutParams) scrap
					.getLayoutParams();
			if (lp == null) {
				return;
			}
